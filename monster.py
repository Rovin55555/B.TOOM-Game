...
# la classe qui va gerer la notion de  monstre
...
class Monster(animation.AnimateSprite):
    ...
    def __init__(self, game, name, size, offset=0):
        ...
        self.rect.x = 1372 + random.randint(0, 300)
    ...
    def forward(self):
        # le deplacement ne se fait que si il n'y a pas de collision avec un groupe de joueurs
        if not self.game.check_collision(self, self.game.all_players):
            self.rect.x -= self.velocity

        # si le monstre est en collision avec le joueur (sinon/else) on inflige des degats au joueur
